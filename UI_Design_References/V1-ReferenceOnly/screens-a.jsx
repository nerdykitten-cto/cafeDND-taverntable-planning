...
// =====================================================================
//  Screens A — Auth · Lobby (DM + Player) · DM Screen
// =====================================================================
...
function LobbyPlayerSection() {
  return (
    <ScreenSection id="lobby-player" num="03" title="Lobby — Player View" refTag="UI Dict §2b"
      ...
      <Sketch title="A · One field, one button"
        tag="minimal join"
        ...
        <Canvas>
          ...
          {[0,1,2,3,4,5].map(i => (
            <g key={i}>
              ...
              <Tx x={115 + i*26} y="118" anchor="middle" className="lbl-display">{i<2?'4':i<4?'_':'_'}</Tx>
            </g>
          ))}
      ...
      <Sketch title="B · Waiting state, focused"
        tag="post-join · DM hasn't started"
      ...
      <Sketch title="C · Persistent join chip"
        tag="code stays visible · room previews"
...
function DMScreenSection() {
  ...
  return (
    <ScreenSection id="dm-screen" num="04" title="DM Screen" refTag="UI Dict §3"
      ...
      <Sketch title="A · Twin sidebars (spec default)"
        tag="left icon rail · right always-on status"
      ...
      <Sketch title="B · Floating glass panels"
        tag="3D-first · panels draggable"
      ...
      <Sketch title="C · Bottom-dock workbench"
        tag="like an OBS studio · 3D fills frame"
        ...
        <Canvas>
          ...
          {/* expanded content for selected (encounter mini) */}
          <Rect x="16" y="196" w="328" h="10" stroke="rgba(240,217,154,0.4)" />
          <Tx x="20" y="204" className="tiny" >▸ Encounter expanded above on click · tabs always visible</Tx>
...
Object.assign(window, { AuthSection, LobbyDMSection, LobbyPlayerSection, DMScreenSection });
