import { Sketch, ScreenSection, Canvas, Rect, Ln, Tx, Ann, Avatar, HPBar, RoomBackdrop, TableMap, Icon } from './ui.jsx';

export function PlayerScreenSection() {
  return (
    <ScreenSection id="player-screen" num="05" title="Player Screen" refTag="UI Dict §4"
      ...
      <Sketch title="A · Bottom bar (spec default)" tag="persistent dock at the foot"
      ...
      <Sketch title="B · Corner radial cluster" tag="immersive · controls tucked"
      ...
      <Sketch title="C · Right-edge tab strip" tag="docked vertically · ergonomic"
...
export function CharacterSheetSection() {
  return (
    <ScreenSection id="character" num="06" title="Character Sheet Panel" refTag="UI Dict §4.2"
      ...
      <Sketch title="A · Two-column scroll" tag="default · dense, complete"
        ...
        <Canvas vb="0 0 360 240">
          ...
          {['Second Wind (1/sr)','Action Surge (1/sr)','Fighting Style: Defense'].map((f,i) => (
            <Tx key={i} x="190" y={184 + i*10} className="tiny">▸ {f}</Tx>
          ))}
      ...
      <Sketch title="B · Tabbed by domain" tag="combat / spells / gear / bio"
      ...
      <Sketch title="C · Single column, sticky header" tag="mobile-shaped · stream of sections"
        ...
        <Canvas vb="0 0 360 240">
          ...
          {[
            ['ABILITIES','▾'],['SAVES','▸'],['SKILLS','▸'],['COMBAT','▾'],
            ['ATTACKS','▾'],['SPELLS','▸'],['EQUIPMENT','▸'],['FEATURES','▸'],['NOTES','▸'],
          ].map(([t,c],i) => (
            <g key={i}>
              <Rect x="108" y={52 + i*18} w="144" h="14" stroke="pencil" fill={c==='▾'?'rgba(192,138,42,0.12)':'none'} />
