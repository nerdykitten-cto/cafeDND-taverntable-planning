...
export function Sketch({ title, tag, children, notes }) {
  return (
...
export function ScreenSection({ id, num, title, refTag, intent, children }) {
  return (
...
export function Canvas({ vb = '0 0 360 220', children, bg = '#fffaee' }) {
  return (
...
export function Rect({ x, y, w, h, fill, stroke = 'ink', sw, dashed, rx = 0, label, labelClass = 'lbl-text', labelDy = 0 }) {
  ...
  return (
    <g>
      <rect x={x} y={y} width={w} height={h} rx={rx}
            className={cls} fill={fill || 'none'} strokeWidth={sw}
            strokeDasharray={dashed ? '3 2' : undefined} />
      {label && (
        <text x={x + w / 2} y={y + h / 2 + 3 + labelDy} textAnchor="middle" className={labelClass}>
          {label}
        </text>
      )}
    </g>
...
export function Circ({ cx, cy, r, fill, stroke = 'ink', dashed }) {
...
export function Ln({ x1, y1, x2, y2, stroke = 'ink', dashed, sw }) {
  const cls = stroke === 'gold' ? 'stroke-gold' : stroke === 'pencil' ? 'stroke-pencil' : 'stroke-ink';
  return <line x1={x1} y1={y1} x2={x2} y2={y2} className={cls} strokeWidth={sw}
               strokeDasharray={dashed ? '3 2' : undefined} />;
}

export function Tx({ x, y, children, className = 'lbl-text', anchor = 'start', style }) {
  return <text x={x} y={y} textAnchor={anchor} className={className} style={style}>{children}</text>;
}

export function Ann({ x1, y1, x2, y2, label, anchor = 'start', dy = 0, gold }) {
  return (
    <g>
      <path d={`M${x1} ${y1} L${x2} ${y2}`} className="ann-leader" />
      <circle cx={x1} cy={y1} r="1.4" fill={gold ? 'var(--gold-deep)' : 'var(--ink)'} />
      <text x={x2 + (anchor === 'end' ? -2 : 2)} y={y2 + dy + 3} textAnchor={anchor}
...
export function PencilGrid({ x, y, w, h, step = 8 }) {
  const lines = [];
...
export function Icon({ kind, cx, cy, size = 6, gold }) {
  const s = size;
...
export function IconRail({ x, y, w, h, icons, active = 0, vertical = true }) {
  const padding = 4;
  const slot = vertical
...
export function Avatar({ cx, cy, r = 6, initial, fill = '#f0d99a' }) {
  return (
    <g>
...
export function HPBar({ x, y, w, h = 3, pct = 0.7, color }) {
...
export function RoomBackdrop({ x, y, w, h }) {
  return (
    <g>
...
export function TableMap({ x, y, w, h, fogPct = 0.4, tokens = [] }) {
  return (
    <g>
      ...
      <rect x={x + w * (1 - fogPct)} y={y} width={w * fogPct} height={h} fill="rgba(50,55,70,0.55)" />
...
export function Tape({ x, y, w = 30, h = 6, rot = -1, text }) {
  return (
